from portal_etl_emr_step_service import FhirSecretObject, FHIR_SECRETS_NAME
...
class EmrStepArgumentBuilder:
    ...
    def with_fhir_custom_job(self, etl_portal_bucket: str, release_id: str, studies: list,
        ...
        kc_exports = f"export KEYCLOAK_CLIENT_ID='{fhir_secret_object.keycloak_client_id}'; export KEYCLOAK_TOKEN_URL='{fhir_secret_object.keycloak_url}';"
        kc_secret = f"""export KEYCLOAK_CLIENT_SECRET=$(aws secretsmanager get-secret-value --secret-id {FHIR_SECRETS_NAME} --output text | grep -o '"keycloak_client_secret":"[^"]*"' | cut -d '"' -f 4);"""

        fhavro_export_args = [
            "aws", "s3", "cp",
            f"s3://{etl_portal_bucket}/jobs/fhavro-export.jar", "/home/hadoop;",
            f"export FHIR_URL='{fhir_url}'; export BUCKET='{etl_portal_bucket}'; {kc_exports} {kc_secret} ",
