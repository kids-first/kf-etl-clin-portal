...
object FhirUtils {
  ...
  def buildFhirClient(tr: TokenRequest, verbose: Boolean): Either[String, GenericClient] = {
    ...
    val fhirClient: GenericClient = ctx.newGenericClient(tr.kcUrl).asInstanceOf[GenericClient]
    if (verbose) {
      loggingInterceptor.setLogRequestSummary(true)
      loggingInterceptor.setLogRequestBody(true)
      fhirClient.registerInterceptor(loggingInterceptor);
    }
    if (isUpgradedServer(tr.kcUrl)) {
