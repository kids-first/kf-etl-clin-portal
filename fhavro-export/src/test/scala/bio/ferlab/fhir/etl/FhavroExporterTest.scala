...
class FhavroExporterTest extends FlatSpec with FhirServerSuite with Matchers {
  ...
  "requestExportFor" should "return a List of Fhir Resource by tag" in {
    ...
    val resources = new FhavroExporter(Config(awsConfig, keycloakConfig, fhirConfig), "re_001", "SD_001").requestExportFor(fhirRequest)
    resources.length shouldBe 2
  }

  "convertFileContentToGenericRecord" should "return a list of Generic Record" in {
    val fhirRequest = FhirRequest("Patient", "kfdrc-patient", None, None, None)
    val fhavroExporter = new FhavroExporter(Config(awsConfig, keycloakConfig, fhirConfig), "re_001", "SD_001")
