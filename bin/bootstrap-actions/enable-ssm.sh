...
##
sudo sh -c 'yum install -y https://s3.amazonaws.com/ec2-downloads-windows/SSMAgent/latest/linux_amd64/amazon-ssm-agent.rpm'
sudo sh -c 'status amazon-ssm-agent >>/tmp/ssm-status.log'
## Update hosts file
...
sudo sh -c 'echo "${v_ipaddr} localhost" >>/etc/hosts'
else
...
sudo sh -c 'cat /etc/hosts >> /tmp/localhost.log'
